  ...
  var buyables = {
    ...
    toys: {
      ...
      "scratching post": {
        ...
        cost: 25,
        description: "A screatching post"
      },
      "laser pointer": {
        name: "Laster pointer",
        cost: 5,
    ...
    for(var name in buyables){
      for(var name1 in buyables[name]){
        if(name1 == item.join(" ")){
          ...
          var bal = (t.balance - buyables[name][name1].cost * quantity) * (dynamic[name1].cur || 1)
          if(bal < 0){
  ...
  for(var name in buyables){
    for(var name1 in buyables[name]){
      if(name1 == item.join(" ")){
        ...
        let cost = buyables[name][commandname].cost * (dynamicItems[name1].cur || 1)
